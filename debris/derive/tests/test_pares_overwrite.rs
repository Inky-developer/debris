use debris_core::{
    error::LangResult, objects::FunctionContext, CompileContext, DebrisObject, ObjectPayload,
    ObjectRef,
};
...
impl Something {
    #[special]
    fn add(_ctx: &FunctionContext) -> Something {
        Something
    }

    #[special]
    fn add(_ctx: &FunctionContext, _: &Something) -> LangResult<Something> {
        Ok(Something)
    }
